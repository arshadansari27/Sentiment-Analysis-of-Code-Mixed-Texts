...
def load_seed():
    count = 0
    lines = []
    for line in open(HOME + '/resources/test2.csv'):
        ...
        if len(u) == 0 or len(v) == 0:
            continue
        lines.append((v, u))
    db.mlines.insert_many([{'line': u, 'output': v} for (u, v) in lines])
    print db.mlines.count()

def start_tagging(other_lang='M'):
    global pool
    print 'Start Tagging...'
    from language_identification import emoticons
    from pos_tag import tag_words_english, tag_words_hindi, tag_words_marathi
    from sentiment import sentiment as stmt
    
    E_TAGS = set(['JJ', 'JJR', 'JJS', 'IN', 'RB', 'RBR', 'RBS', 'VB', 'VBD', 'VBG', 'VBN', 'VBP', 'VBZ', 'WRB'])
    M_TAGS = set(['V', 'VM', 'VAUX', 'INTF', 'NEG', 'QT', 'QTC', 'QTF', 'QTO'])

    count = 0
    lines = []
    for obj in db.mlines.find({}):  # {"_id" : ObjectId("575ca51699eb7b1c9bee02a9")}
        line = obj['line']
    ...
    for _id, line in lines:
        ...
        db.mlines.update_one({'_id': _id}, {'$set': {'words': words, 'epos': pos_s['E'], 'opos': pos_s[other_lang], 'status': 'done'}}, upsert=False)
    ...
    for _id, words, epos, opos in lines:
        ...
        for score in scores:
            if score[3][0] > 0. or score[3][1] > 0.:
                ...
                count += 1
            total += 1
        db.mlines.update_one({'_id': _id}, {'$set': {'pos_tags': complete_tag, 'sores': scores, 'status': 'assigned'}}, upsert=False)
    print total, count
...
def collect_words():
    ...
    for _id, scores in lines:
        ...
        for word in words:
            ...
            db.mwords.insert({'word': word[0], 'lang': word[1], 'polarity': word[2]})
...
def generate_features():
    ...
    for obj in db.mlines.find({}):
        ...
        output = obj['output']
        db.mfeatures.insert({'input': words, 'output': output}) 
...
def train_test():
    data = []
    for obj in db.mfeatures.find({}):
        _input = obj['input']
        _output = obj['output']
        data.append((_input, _output))
    for train_percent in [.2, .4, .5, .6, .75, .8]:
        for i in xrange(1):
            random.shuffle(data)
            train_data = []
            train_labels = []
            ...
            for _input, _output in data:
                ...
                if random.random() > train_percent:
                    ...
                    if (
                            (summation > 0. and _output == 'positive') or 
                            (summation is 0. and _output == 'neutral') or 
                            (summation < 0. and _output == 'negetive')):
                        summation_accuracy += 1.
            ...
            print len(prediction_liblinear), len(test_labels), len(train_labels)
            accuracy = 0.
            for i, p in enumerate(prediction_liblinear):
                if p == test_labels[i]:
                    accuracy += 1.
            t2 = time.time()
            ...
            print "Results for LinearSVC With Training Percent", train_percent, 'with accuracy', accuracy, (accuracy / len(prediction_liblinear)), 'and by summation', summation_accuracy, (summation_accuracy / len(prediction_liblinear)), 'out of', len(prediction_liblinear)
